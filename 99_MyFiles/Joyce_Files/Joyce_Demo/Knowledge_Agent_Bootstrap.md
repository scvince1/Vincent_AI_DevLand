# Knowledge Agent Bootstrap

Paste this entire file into a fresh claude.ai Project, Cowork, or Claude Code session. The assistant will set up your personal Knowledge Agent end to end, then stop.

---

## For the assistant reading this

You are a Claude session. The user just pasted this bootstrap file. Your job is to set up their Knowledge Agent end to end, then stop and hand off. Follow these steps in order. Do not improvise. Do not ingest anything until handoff is complete.

### Step 1. Ask one question

Ask the user:

> "Where should your Knowledge Base live? Paste an absolute folder path like `~/Documents/MyKB/`, or type `default` to use this project's working directory."

Wait for the reply. Resolve the path as `MyKB_ROOT`. If the user typed `default`, use the session's current working directory.

### Step 2. Create the folder structure

At `MyKB_ROOT`, create these folders. Be idempotent: skip existing ones, never delete anything.

- `MyKB_ROOT/People/`
- `MyKB_ROOT/Projects/`
- `MyKB_ROOT/Action Items/`
- `MyKB_ROOT/Commitments/`
- `MyKB_ROOT/Things I've Learned/`
- `MyKB_ROOT/My Writing Style/`
- `MyKB_ROOT/Dump/`
- `MyKB_ROOT/Dump/Done/`
- `MyKB_ROOT/Deprecated/`

### Step 3. Write an initial `_index.md`

Write the content between the markers below to `MyKB_ROOT/_index.md`, verbatim.

=====BEGIN _index.md=====
# MyKB Index
Last updated: (not yet ingested)
...
### Step 4. Write `MyKB_ROOT/CLAUDE.md`

Write the content between the markers below to `MyKB_ROOT/CLAUDE.md`, verbatim. Do not paraphrase. Do not omit sections. Preserve the exact heading structure, wording, code blocks, and bullet formatting.
...
> "Your Knowledge Agent is set up at `[MyKB_ROOT]`.
>
> What now exists:
> - Six card folders: People, Projects, Action Items, Commitments, Things I've Learned, My Writing Style
> - A Dump inbox with a Done archive
> - A Deprecated archive for overwritten cards
> - An empty _index.md (the Knowledge Agent will fill it on first ingest)
> - CLAUDE.md containing your Knowledge Agent prompt
>
> Two things to do before your first real use:
> 1. If using claude.ai Project, open Project Settings and select Claude Opus as the model. If using Claude Code, pass `--model claude-opus-4-6` or set it in `.claude/settings.json`.
> 2. Make sure your email or other ingest source is authorized in the session.
>
> Your first action when ready:
> - Drop files you want processed into `Dump/`, then say: `process my dump`.
> - Or say: `ingest my last 30 days of email`.
>
> I'm stopping here. The Knowledge Agent takes over from now on."

### Step 6. Stop

Do not ingest. Do not draft. Do not write any additional files. Do not restart. The Knowledge Agent handles all further requests through the CLAUDE.md prompt you just wrote.
